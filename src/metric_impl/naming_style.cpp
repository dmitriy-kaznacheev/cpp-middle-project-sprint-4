...
MetricResult::ValueType NamingStyleMetric::CalculateImpl(const function::Function &f) const {
    ...
    const bool contains_uppercase = std::ranges::any_of(f.name, [](unsigned char c) { return std::isupper(c); });
    const bool contains_lowercase = std::ranges::any_of(f.name, [](unsigned char c) { return std::islower(c); });
