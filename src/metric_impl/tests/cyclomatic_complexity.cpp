...
TEST_F(cyclomatic_complexity, comments) {
  ...
  EXPECT_EQ(std::get<int>(metric.CalculateImpl(f)), 0);
}
...
TEST_F(cyclomatic_complexity, if) {
  ...
  EXPECT_EQ(std::get<int>(metric.CalculateImpl(f)), 1);
}

TEST_F(cyclomatic_complexity, loops) {
  auto f = get_first_function("loops.py"s);
  EXPECT_EQ(std::get<int>(metric.CalculateImpl(f)), 3);
}
...
TEST_F(cyclomatic_complexity, nested_if) {
  ...
  EXPECT_EQ(std::get<int>(metric.CalculateImpl(f)), 5);
}
